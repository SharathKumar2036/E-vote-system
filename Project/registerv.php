...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="loginv.php"><span class="glyphicon glyphicon-log-in"></span> Login</a></li>
    </ul>
  </div>
</nav>
<br><br><br>
...
if(isset($_POST["signin"]))
{
	...
	if(!empty($name) && !empty($id) && !empty($ph_no) && !empty($password) && !empty($cpass)){
		if($password==$cpass){
			$create="CREATE TABLE IF NOT EXISTS Project.voterSignup ( name VARCHAR(50) NOT NULL , id VARCHAR(20) NOT NULL , email VARCHAR(30) NULL , ph_no BIGINT(15) NOT NULL , password VARCHAR(15) NOT NULL , cpassword VARCHAR(15) NOT NULL,status INT(11) NULL)";
			mysqli_query($conn,$create);
			$s= "select * from Project.voterSignup where id ='$id'";
			$result=mysqli_query($conn,$s);
			if(mysqli_num_rows($result) == 1){
				echo '<script>alert("Already Registered!!")</script>';
			}
			else{
				$insert="insert into Project.voterSignup (name,id,email,ph_no,password,cpassword) values('$name','$id','$email','$ph_no','$password', '$cpass')";
